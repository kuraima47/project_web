...
module.exports = (io) => {
  io.on('connection', (socket) => {
    ...
    if (!decoded) {
      console.log("déconnecté, mauvais token pixelwar");
      socket.disconnect();
      return;
    }
    const socketId = socket.id;
    const userId = decoded.id;
    console.log('Un utilisateur est connecté pour la pixelwar :', socketId);
    addUserToSocketMap(userId, socketId);

    // Déconnexion de l'utilisateur
    socket.on('disconnect', () => {
      console.log('Un utilisateur s\'est déconnecté de la pixelwar :', socketId);
